// Original Shader By David Hoskins - 2013 https://www.shadertoy.com/view/4slGD4
// License Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License.
// A ray-marched version of my terrain renderer which uses
// streaming texture normals for speed:-
// Modified by Ricku34 for ShaderElement sample  https://github.com/Ricku34/ShaderElement

uniform float time;
uniform vec2 resolution;
uniform mat4 CameraView;

vec3 CamPos = vec3(CameraView[3].xyz);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	for (int i = 0; i < 7; i++)
	{
		h += Terrain(CamPos.xz) * f;
		f -= .1;
	}
	CamPos.y = max((h*.25)+3.5, 1.0);
	
	vec3 cw = normalize(vec3(CameraView[0][2],CameraView[1][2],CameraView[2][2]));
	vec3 cp = vec3(CameraView[0][1],CameraView[1][1],CameraView[2][1]);
	vec3 cu = normalize(cross(cw,cp));
	vec3 cv = normalize(cross(cu,cw));
	vec3 rd = normalize( uv.x*cu + uv.y*cv + 1.5*cw );

	#ifdef STEREO
	CamPos += .45*cu*isCyan; // move camera to the right - the rd vector is still good
	#endif

	vec3 col;
	float distance;
	if( !Scene(CamPos,rd, distance, fragCoord) )
	{
